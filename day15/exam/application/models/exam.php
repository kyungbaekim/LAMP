...
class Exam extends CI_Model {
  ...
  function get_all_travel_of_user($id){
    $query = "SELECT users.id AS user_id, users.name, trip_schedules.id AS trip_id, trip_schedules.destination, trip_schedules.startdate, trip_schedules.enddate, trip_schedules.description, join_trips.user_id AS join_id, users2.name AS joiner FROM users
              ...
              WHERE trip_schedules.user_id = '{$id}' or join_trips.user_id= '{$id}'";
              // WHERE trip_schedules.id IS NOT NULL";
    return $this->db->query($query)->result_array();
  }

  function get_other_user_travel($id){
    $query = "SELECT users.id AS user_id, users.name, trip_schedules.id AS trip_id, trip_schedules.user_id AS trip_creator, trip_schedules.destination, trip_schedules.startdate, trip_schedules.enddate, trip_schedules.description, join_trips.user_id AS join_id, users2.name AS joiner FROM users
              LEFT JOIN trip_schedules
              ON users.id = trip_schedules.user_id
              LEFT JOIN join_trips
              ON trip_schedules.id = join_trips.trip_schedule_id
              LEFT JOIN users as users2
              ON join_trips.user_id = users2.id
              WHERE trip_schedules.user_id != '{$id}' and join_trips.user_id != '{$id}'";
    return $this->db->query($query)->result_array();
  }
  ...
  function get_travel_by_id($id){
    $query = "SELECT users.id AS user_id, users.name, trip_schedules.id AS trip_id, trip_schedules.destination, trip_schedules.startdate, trip_schedules.enddate, trip_schedules.description, join_trips.user_id AS join_id FROM users
              ...
              LEFT JOIN join_trips
              ON trip_schedules.id = join_trips.trip_schedule_id
              WHERE trip_schedules.id = '{$id}' or join_trips.user_id = '{$id}'";
    return $this->db->query($query)->row_array();
  }
