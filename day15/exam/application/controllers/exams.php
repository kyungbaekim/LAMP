...
class Exams extends CI_Controller {
  ...
  public function login(){
	...
	public function travels(){
		$this->load->model('Exam');
		$user_trips = $this->Exam->get_all_travel_of_user($this->session->userdata['user_id']);
		$other_trips = $this->Exam->get_other_user_travel($this->session->userdata['user_id']);
		$this->load->view('/dashboard', array('trips' => $user_trips, 'other_trips' => $other_trips));
		// if($trips){
    ...
    $this->form_validation->set_rules('from', 'START DATE', 'required|callback_checkStartDate');
		...
		$this->form_validation->set_message('checkStartDate', 'Your start date must be a future date.');
		$this->form_validation->set_message('checkEndDate', 'Your end date must be a future date than start date.');
    if($this->form_validation->run() == FALSE){
